...
for i in range(0, 500):
	...
	# ランダムに1個取る
	fileNameJsonThread = listFileNameJsonThread[random.randint(0, len(listFileNameJsonThread) - 1)]
	...
	with open(fileNameJsonThread) as f:
		...
		for jsonResponse in jsonThread:
			...
			if len(tensors) == 10:
				...
				npx = np.array([x])
				npy = np.array([y])
				model.fit(npx, npy, epochs=1)
	
	# モデル保存
	model.save_weights("./main.model")
